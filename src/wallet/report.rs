...
pub struct SwapReport {
    ...
    pub recovery_txids: Option<String>,
...
impl SwapReport {
    ...
    pub fn maker_recovery(
        ...
        Self::maker_report(
            Self {
                ...
                incoming_contract_txid: Some(incoming_contract_txid),
                outgoing_contract_txid: Some(outgoing_contract_txid),
                recovery_txids: Some(recovery_txids),
                timelock,
                ..Default::default()
            },
            0.0,
    ...
    pub fn emit_taker_recovery_report(
        ...
        let recovery_txids_str = if recovery_txids.is_empty() {
            None
        } else {
            Some(recovery_txids.join(", "))
        };
        ...
        let report = Self::taker_report(SwapReport {
            ...
            outgoing_amount,
            network,
            recovery_txids: recovery_txids_str,
    ...
    pub fn save_to_disk(&self, data_dir: &Path) -> std::io::Result<()> {
        ...
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default();
        let timestamp = format_unix_timestamp_utc(now.as_secs(), now.subsec_millis());
    ...
    pub fn print(&self) {
        ...
        if let Some(ref txid) = self.recovery_txids {
