...
impl Taker {
    ...
    pub fn do_coinswap(
            ...
            .negotiate_with_makers_and_coordinate_sweep(&outgoing_signed_contract_transactions)
        {
            ...
            Err(e) => {
                ...
                self.emit_failed_swap_report(swap_start_time, &initial_utxoset, &e);
    ...
    // Helper function for generating failed swap report from current ongoing state.
    fn emit_failed_swap_report(
        &self,
        swap_start_time: std::time::Instant,
        initial_utxoset: &[ListUnspentResultEntry],
        err: &TakerError,
    ) {
        let swap_state = self.ongoing_swap_state.clone();
        let _ = self.generate_swap_report(
            &swap_state,
            swap_start_time,
            initial_utxoset,
            SwapStatus::Failed,
